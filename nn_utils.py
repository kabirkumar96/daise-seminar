...
def train(model, epochs, optimizer, trainloader, valloader, criterion, train_fd = False):
    # Early stopping
    patience = 10
...
def cross_validation(nn_arch, dataset_train, dataset_test, epochs = 50, criterion = nn.MSELoss(), k_folds = 5, train_fd = False, batch_size = 32):
    """_summary_
    ...
    return  {'tr': np.mean(results['tr']), 'val': np.mean(results['val']), 'test': np.mean(results['test']), 'aic': np.mean(results['aic'])}
